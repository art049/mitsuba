...
#include <mitsuba/render/dosppmparticleproc.h>
...
DOSPPMParticleProcess::DOSPPMParticleProcess(EMode mode, size_t workCount, size_t granularity,
...
DOSPPMParticleProcess::~DOSPPMParticleProcess() {
	delete m_progress;
}

ParallelProcess::EStatus DOSPPMParticleProcess::generateWork(WorkUnit *unit, int worker) {
...
void DOSPPMParticleProcess::increaseResultCount(size_t resultCount) {
	LockGuard lock(m_resultMutex);
	m_receivedResultCount += resultCount;
	m_progress->update(m_receivedResultCount);
}

DOSPPMParticleTracer::DOSPPMParticleTracer(int maxDepth, int rrDepth, bool emissionEvents)
	: m_maxDepth(maxDepth), m_rrDepth(rrDepth), m_emissionEvents(emissionEvents) { }

DOSPPMParticleTracer::DOSPPMParticleTracer(Stream *stream, InstanceManager *manager)
...
void DOSPPMParticleTracer::serialize(Stream *stream, InstanceManager *manager) const {
	stream->writeInt(m_maxDepth);
	stream->writeInt(m_rrDepth);
	stream->writeBool(m_emissionEvents);
}

ref<WorkUnit> DOSPPMParticleTracer::createWorkUnit() const {
	return new RangeWorkUnit();
}

void DOSPPMParticleTracer::prepare() {
...
void DOSPPMParticleTracer::process(const WorkUnit *workUnit, WorkResult *workResult,
		const bool &stop) {
...
void DOSPPMParticleTracer::handleSurfaceInteraction(int depth, int nullInteractions,
	bool delta, const Intersection &its, const Medium *medium,
	const Spectrum &weight) { }

void DOSPPMParticleTracer::handleMediumInteraction(int depth, int nullInteractions,
